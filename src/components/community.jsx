...
import { useMobile, NavBar, PageBackground, ImgPlaceholder } from './shared';

export function Mirage({ onNav }) {
...
export function Announcements({ onNav }) {
  const isMobile = useMobile();
  const posts = [
...
export function LostFound({ onNav }) {
  const isMobile = useMobile();
